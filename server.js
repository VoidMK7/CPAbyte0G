...
const APP_URL = (process.env.RENDER_EXTERNAL_URL || process.env.PUBLIC_APP_URL || '').replace(/\/$/, '');
...
async function postTask(task) {
  ...
  return telegram('sendMessage',{chat_id:process.env.CHANNEL_ID,text,parse_mode:'Markdown',reply_markup:{inline_keyboard:[[{text:'📤 Submit Proof',web_app:{url:APP_URL}}]]}});
}
...
app.post('/telegram/webhook',async(req,res)=>{try{const m=req.body?.message;if(!m?.from?.id)return res.sendStatus(200);const tid=m.from.id;const role=await roleOf(tid);const text=m.text||'';if(text.startsWith('/start'))await telegram('sendMessage',{chat_id:tid,text:'Welcome to HillsByte Proof Bot. Open the mini app to complete tasks and submit proof.',reply_markup:{inline_keyboard:[[{text:'🚀 Open Proof App',web_app:{url:APP_URL}}]]}});else if(text.startsWith('/review')&&['admin','reviewer'].includes(role))await telegram('sendMessage',{chat_id:tid,text:'Reviewer access is available in the Proof Mini App.',reply_markup:{inline_keyboard:[[{text:'🧾 Open Review Center',web_app:{url:`${APP_URL}?review=1`}}]]}});else if(text.startsWith('/admin')&&role==='admin')await telegram('sendMessage',{chat_id:tid,text:'Admin access is available in the Proof Mini App.',reply_markup:{inline_keyboard:[[{text:'⚙️ Open Admin',web_app:{url:`${APP_URL}?admin=1`}}]]}});res.sendStatus(200);}catch(e){console.error(e);res.sendStatus(200);}});

app.use(express.static(path.join(ROOT, 'public')));
app.use((req,res,next)=>{if(req.path.startsWith('/api/')||req.path==='/telegram/webhook')return next();res.sendFile(path.join(ROOT, 'public', 'index.html'));});

app.listen(port,async()=>{console.log(`HillsByte Proof Bot listening on ${port}`);if(process.env.BOT_TOKEN && APP_URL){try{await telegram('setWebhook',{url:`${APP_URL}/telegram/webhook`});console.log(`Telegram webhook configured: ${APP_URL}/telegram/webhook`);}catch(e){console.error('Webhook setup:',e.message);}}});
