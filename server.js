...
// Frontend files are kept at the repository root for simple GitHub/Render deployment.
// Serve only the intended public frontend assets; do not expose the repository root.
app.get("/", (_req, res) => res.sendFile(path.join(__dirname, "index.html")));
app.get("/app.js", (_req, res) => res.sendFile(path.join(__dirname, "app.js")));
app.get("/app.css", (_req, res) => res.sendFile(path.join(__dirname, "app.css")));
...
if (bot) {
  bot.setMyCommands([
    { command: "start", description: "Open Proof Center" },
    { command: "pending", description: "View pending proofs" },
    { command: "dashboard", description: "View dashboard" },
    { command: "moderators", description: "Manage moderators (owner)" }
  ]).catch(console.error);

  async function sendAdminMenu(chatId) {
    const actor = await getModerator(chatId);
    if (!actor) return bot.sendMessage(chatId, "⛔ You are not authorized to use HillsByte Proof Center.");
    const keyboard = [
      [{ text: "📊 Dashboard", callback_data: "dash" }, { text: "🖼 Proofs", callback_data: "proofs:pending" }],
      [{ text: "🕐 Pending", callback_data: "proofs:pending" }, { text: "✅ Approved", callback_data: "proofs:approved" }],
      [{ text: "❌ Rejected", callback_data: "proofs:rejected" }]
    ];
    if (actor.role === "OWNER") keyboard.push(
      [{ text: "👮 Moderators", callback_data: "mods" }, { text: "🗑 Reviewed", callback_data: "reviewed" }]
    );
    return bot.sendMessage(chatId, "🛡 *HillsByte Proof Center*\n\nFast, private proof review.", {
      parse_mode: "Markdown", reply_markup: { inline_keyboard: keyboard }
    });
  }

  bot.onText(/^\/start$/, msg => sendAdminMenu(msg.chat.id));
  ...
  bot.onText(/^\/pending$/, async msg => {
    ...
    for (const p of data) await bot.sendMessage(msg.chat.id, `🕐 ${p.task_name}\nID: ${p.submission_id}\nUser: ${p.user_name || p.user_telegram_id}`, {
      reply_markup:{inline_keyboard:[[{text:"Review",callback_data:`review:${p.id}`}]]}
    });
  });
  bot.onText(/^\/moderators$/, async msg => {
    ...
    bot.sendMessage(msg.chat.id, "👮 Moderators\n\n" + (data||[]).map(m=>`${m.active?"🟢":"⚪"} ${m.full_name||"Unknown"} — ${m.telegram_id}`).join("\n"));
  });

  bot.on("callback_query", async q => {
    const chatId = q.message.chat.id;
    try {
      const actor = await getModerator(q.from.id);
      if (!actor) return bot.answerCallbackQuery(q.id, { text: "Not authorized.", show_alert: true });
      const [action, value] = String(q.data || "").split(":");
      await bot.answerCallbackQuery(q.id);

      if (action === "dash") return bot.sendMessage(chatId, "Use /dashboard for current proof counts.");
      if (action === "proofs") {
        const { data } = await supabase.from("proof_submissions").select("*").eq("status",value).order("submitted_at",{ascending:false}).limit(10);
        if (!data?.length) return bot.sendMessage(chatId, `No ${value} proofs.`);
        for (const p of data) bot.sendMessage(chatId, `${value==="pending"?"🕐":value==="approved"?"✅":"❌"} ${p.task_name}\n${p.submission_id}`, {reply_markup:{inline_keyboard:[[{text:"Open",callback_data:`review:${p.id}`}]]}});
      }
      if (action === "review") {
        const p = await getProof(value);
        if (!p) return bot.sendMessage(chatId,"Proof not found.");
        const url = await createSignedUrl(p.screenshot_path);
        const buttons = p.status === "pending" ? [[
          {text:"✅ Approve",callback_data:`approve:${p.id}`},
          {text:"❌ Reject",callback_data:`reject:${p.id}`}
        ]] : [];
        await bot.sendPhoto(chatId, url, {caption:`${p.task_name}\nTask ID: ${p.task_id}\nUser: ${p.user_name||"-"}\nTelegram ID: ${p.user_telegram_id}\nSubmission: ${p.submission_id}\nStatus: ${p.status}`, reply_markup:{inline_keyboard:buttons}});
      }
      if (action === "approve") {
        const r = await reviewProof(value, q.from.id, "approved");
        bot.sendMessage(chatId, r.callback_error ? "⚠️ Approved, but Base44 callback failed. Check logs." : "✅ Proof approved and Base44 notified.");
      }
      if (action === "reject") {
        bot.sendMessage(chatId, "Reply with /reject <submission_uuid> <reason_type> <optional reason> to reject. Types: " + Object.keys(REASONS).join(", "));
      }
      if (action === "mods" && actor.role === "OWNER") return bot.sendMessage(chatId, "Moderator management is available in the mobile web Proof Center.");
      if (action === "reviewed" && actor.role === "OWNER") return bot.sendMessage(chatId, "Bulk deletion is protected and available in the web Proof Center after explicit confirmation.");
    } catch (e) { console.error(e); }
  });

  bot.onText(/^\/reject\s+(\S+)\s+(\S+)(?:\s+([\s\S]+))?$/, async (msg, match) => {
    const actor = await getModerator(msg.from.id);
    if (!actor) return;
    try {
      const r = await reviewProof(match[1], msg.from.id, "rejected", match[2], match[3] || "");
      bot.sendMessage(msg.chat.id, r.callback_error ? "❌ Rejected, but Base44 callback failed." : "❌ Proof rejected and Base44 notified.");
