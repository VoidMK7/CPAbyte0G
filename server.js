...
const BOT_MODE = String(process.env.BOT_MODE || "webhook").toLowerCase();
const bot = process.env.TELEGRAM_BOT_TOKEN
  ? new TelegramBot(process.env.TELEGRAM_BOT_TOKEN, { polling: BOT_MODE === "polling" })
  : null;
...
// Explicit Web App entry route. This prevents Telegram from receiving Express
//'s default "Cannot GET /" response when the Render service is opened at /.
app.get("/", (_req, res) => {
  const preferred = path.join(PUBLIC_DIR, "index.html");
...
app.post("/telegram/webhook", (req, res) => {
  if (bot && BOT_MODE !== "polling") bot.processUpdate(req.body);
  res.sendStatus(200);
});
...
if (bot) {
  ...
  // Keep the bot menu useful: the Web App is the actual dashboard/review UI.
  const webButton = () => webUrl ? ({ inline_keyboard: [
    [{ text: "🚀 Open Proof Center", web_app: { url: webUrl } }],
    [{ text: "🧾 Proofs", web_app: { url: webUrl } }, { text: "📊 Dashboard", web_app: { url: webUrl } }]
  ] }) : undefined;
  ...
  if (webUrl) {
    ...
    if (BOT_MODE !== "polling") {
      telegramApi("setWebhook", { url: `${webUrl}/telegram/webhook`, allowed_updates: ["message"] })
        .then(() => console.log(`[telegram] webhook configured: ${webUrl}/telegram/webhook`))
