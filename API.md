...
All Telegram Web App requests are authenticated server-side from the signed `initData` in:

`X-Telegram-Init-Data`

## POST /api/user/proofs

Telegram users can create a pending proof. The server authenticates the Telegram user and looks up the task through the main app. The browser cannot choose the authoritative reward or task name.

## POST /api/proofs

Server-to-server Base44/main-app endpoint for creating a pending proof.


Content-Type: `multipart/form-data`

Server-to-server fields:
